...
app.post('/api/schedule-post', async (req, res) => {
  try {
    ...
    const platforms = Array.isArray(body.platforms) && body.platforms.length
      ? body.platforms
      : [{
        platform: String(body.platform || 'Social').trim(),
        accountId: String(body.accountId || body.socialAccountId || body.id || '').trim(),
      }];
    ...
    if (!normalizedPlatforms.length) {
      return res.status(400).json({ error: 'Choose at least one connected account before scheduling.' });
    }

    const content = String(body.content || body.caption || body.transcript || '').trim();
    const title = String(body.title || content || 'Scheduled post').trim().slice(0, 160);
    const scheduledAt = normalizeScheduledFor(body.scheduledFor || body.scheduledAt || '');

    const post = {
      id: crypto.randomBytes(8).toString('hex'),
      platform: normalizedPlatforms.map((item) => item.platform).filter(Boolean).join(', ') || 'Social',
      platforms: normalizedPlatforms.map((item) => item.platform).filter(Boolean),
      accounts: normalizedPlatforms.map((item) => item.accountId).filter(Boolean),
      title,
      transcript: content,
      postType: String(body.postType || 'text'),
      mediaUrl: String(body.mediaUrl || ''),
      mediaType: String(body.mediaType || ''),
      scheduledAt: scheduledAt || null,
      postedAt: scheduledAt ? scheduledAt : null,
...
app.get('/api/content-lab/calendar', async (req, res) => {
  try {
    ...
    const calendar = (Array.isArray(workspace.posts) ? workspace.posts : []).filter((post) => {
      const dateValue = post.scheduledAt || post.postedAt;
    ...
    return res.json({ calendar, raw: {} });
  } catch (error) {
    return res.status(500).json({ error: `Calendar lookup failed: ${error.message}` });
  }
});
...
app.get('/api/reports/competitors', async (req, res) => {
  try {
    ...
    return res.status(501).json({ error: 'Competitor reports are not available in the current integration.', requestedHandles: handles });
  } catch (error) {
...
app.post('/api/scheduling/generate-caption', async (req, res) => {
  try {
    ...
    return res.status(501).json({ error: 'Caption generation is handled by the local AI endpoints.' });
  } catch (error) {
